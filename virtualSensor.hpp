...
using BasicVariantType = std::variant<std::string, int64_t, uint64_t, double,
      int32_t, uint32_t, int16_t, uint16_t, uint8_t, bool, std::vector<uint8_t>>;

using InterfaceMap =
     std::map<
         std::string,
         std::map<std::string, BasicVariantType>>;

using ManagedObjectType = std::map<
...
class VirtualSensor : public ValueObject
{
  public:
    ...
    VirtualSensor(sdbusplus::bus::bus& bus, const char* objPath,
        ...
        bus(bus), name(name)
    {
        initVirtualSensor(ifacemap, objPath, type);
    }
  ...
  private:
    ...
    std::string getThresholdType(std::string direction,
                        std::string name, int severity);
    /** @brief calculate median value from sensors */
    double calculateValue(std::string sensortype, const VirtualSensor::ParamMap& paramMap);
...
class VirtualSensors
{
  ...
  private:
    ...
    ManagedObjectType getConfigFromDBus();
    ...
    void createVirtualSensorsFromDBus();
