...
static constexpr auto vsConfigIntfPrefix =
...
void VirtualSensor::updateVirtualSensor()
{
    ...
    double val;
    auto itr = std::find(calculationTypes.begin(), calculationTypes.end(), exprStr);
...
double VirtualSensor::calculateValue(std::string calc, const VirtualSensor::ParamMap& paramMap)
{
    auto itr = std::find(calculationTypes.begin(), calculationTypes.end(), calc);
    if (itr == calculationTypes.end())
      return  std::numeric_limits<double>::quiet_NaN();
    else if (calc == "modifiedMedian")
...
double VirtualSensor::calculateModifiedMedianValue(const VirtualSensor::ParamMap& paramMap)
{
    ...
    for (auto& param : paramMap)
    {
        ...
        if (auto var = symbols.get_variable(name))
        {
            if (std::isnan(var->ref()))
              continue;
            values.push_back(std::clamp(var->ref(), ValueIface::minValue(), ValueIface::maxValue()));
        }
    }
...
std::string VirtualSensor::getThresholdType(std::string direction,
                        std::string name, int severity)
{
  // do we want to check and/or enforce the lower threshold being lower than the upper?
...
const std::string getCalculationType(std::string interface)
{
                                      ...
                                      const std::string& sensorType)
{
    ...
    const std::string  vsCParamsIntf =
        vsConfigIntfPrefix + sensorType + vsCParamsIntfSuffix;
    const std::string vsDBusParamsIntf =
        vsConfigIntfPrefix + sensorType + vsDBusParamsIntfSuffix;
    const std::string vsThresholdsIntf =
        vsConfigIntfPrefix + sensorType + vsThresholdsIntfSuffix;
    ...
    for (const auto& [interface, propertyMap] : interfaceMap)
    {
        ...
        if (interface.find(vsCParamsIntf) != std::string::npos)
        {
            ...
            if (value != std::numeric_limits<double>::quiet_NaN() && itr != propertyMap.end())
            {
        ...
        else if (interface.find(vsDBusParamsIntf) != std::string::npos)
        {
            ...
            if (itr != propertyMap.end() && paramitr != propertyMap.end())
            {
                ...
                if (!sensorType.empty() && !sensorName.empty() && !paramName.empty())
                {
                    ...
                    symbols.create_variable(paramName);
                    paramMap.emplace(std::move(paramName), std::move(paramPtr));
                    std::cerr << "param created for sensor " << sensorName << "\n";
                }
                else
                {
                     log<level::ERR>("Invalid parameter",
                              entry("TYPE = %s", sensorType.c_str()),
                              entry("PARAM NAME = %s", paramName.c_str()),
        ...
        {
            std::string direction = "", name = "", threshold = "";
            int severity = -1;
            double value;
            auto itr = propertyMap.find("Direction");
            // should we ignore direction completely?
            if (itr != propertyMap.end())
            {
            ...
            if (itr != propertyMap.end())
            {
                severity = std::get<double>(itr->second);
            }
            ...
            if ((threshold = getThresholdType(direction, name, severity)) == "")
            {
                throw std::invalid_argument(
        ...
        else if (interface.find(vsConfigIntfPrefix) !=
                 std::string::npos)
        {
            /* Get expression string */
...
ManagedObjectType VirtualSensors::getConfigFromDBus()
{
...
{
    auto objects = getConfigFromDBus();
    ...
    for (const auto& [path, interfaceMap] : objects)
    {
        ...
        for (const auto& [interface, propertyMap] : interfaceMap)
        {
            for (const char* type : calculationTypes)
            {
                ...
                if (itr != propertyMap.end() && type == std::get<std::string>(itr->second))
                {
                    sensorType = type;
                }
            }
        ...
        if (!name.empty() && !sensorType.empty())
        {
            ...
            else
            {
                ...
                {
                    log<level::ERR>(
                        "Failed to set up virtual sensor",
         ...
         {
             log<level::ERR>("Sensor type or name not found from entity manager");
         }
    }
}
    ...
    for (const char* type : calculationTypes)
    {
        auto match = std::make_unique<sdbusplus::bus::match::match>(
            ...
            "type='signal',member='PropertiesChanged',path_namespace='" +
                std::string(inventoryPath) + "',arg0namespace='" + vsConfigIntfPrefix + type + "'",
            eventHandler);
